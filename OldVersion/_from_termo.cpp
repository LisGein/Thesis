...
extern float P[MaxElemGasFromDB];	// для удобства отладки
...
double	*MK=0;	// 'MK' молекулярная масса конд. вещ.
...
//		STOP
	...
	if ((N!=0)&&(REJ!=REJ1))
		goto ERR;
//LR:
	if (REJ=="S   ")	// то ли вывести, то ли вычислить - теплофизические параметры
		FS=TRUE;
	if (REJ=="PRINT")
		BP=1;
	if ((FS)&&(BP==1))
...
// My code end
	...
	//перепиши расчет конденсатов с использованием структуры "cdG" дурик
	R = T/1000;
	C = log(R)/1000;
	if (RK != 0)
	{
	...
	float &E,double &IS,float &T,float &PP,type_CriticalSection &_ParamInCriticalSection)
{
	...
	_ParamInCriticalSection.B				= BTMIN;			// Расходный комплекс
	_ParamInCriticalSection.PYD			= PYD;
	_ParamInCriticalSection.PYDP			= PYDP;
	_ParamInCriticalSection.F				= BT/PPK;		// Удельная площадь сечения
	...
	float &E,double &IS,float &T,float &PP,type_Section &_ParamInSection)
{
	...
	_ParamInSection.B				= BTMIN;			// Расходный комплекс
	_ParamInSection.PYD			= PYD;
	_ParamInSection.PYDP			= PYDP;
	_ParamInSection.F				= BT/PPK;		// Удельная площадь сечения
	_ParamInSection.F_div_FKP	= BT/BTMIN;		// Геом. степень расширения сопла
		...
		BOOL &F3C,BOOL &FP3,float &P1,float &T,float &T2,float &T1,float &PP,float &P2) 
{
	...
	BT = BT * PPK/PP;			// BT = ((847.9/MS)*T/(91.48*sqrt(HK-IS)))+PPK/PP
	E = RR;
	RR = FA-BT/BTMIN;	// BT/BTMIN - Геом. степень расширения сопла
	RR = RR*DA;			// DA - направление
	if ( (fabs(RR) > (0.001*FA)) && ((fabs(PY-PX)) >= (0.01*PY))) {
		// MENJE
		F0T=CKACH=FALSE;
//LR:	_Func_if_FF__PX_PY_FX_FY_PPA(PP);// Обработка различных сечений сопла
		if (RR>0)
			{ PY=PP; FY=RR; }	// Если текущая площадь меньше искомой
		else
			{ PX=PP; FX=RR; }	// Если текущая площадь превышает искомую
		if ((FX*FY)==0) {	
			// Сюда будет вход пока текущая площадь 
			// не превысит искомую и не станет меньше искомой
			if (RR<0)
...
//LL:	_Func__T2_PPA_PP_P1_P2_T_FP3_T1(FT,F3C,FP3,T3AM,P1,T,T2,T1,PP,P2);
		...
		T1=0;
		PP=PPA;
		return FALSE;	// FALSE - продолжать расчет при новой температуре и давлении
	}
	PX = PY;
	RR = E;
	return TRUE;
